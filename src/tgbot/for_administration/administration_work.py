...
from src.tgbot.keyboard import (get_choose_weekday_kb, get_choose_group_kb, choose_lessons_kb,
...
async def to_auditory(callback: CallbackQuery, state: FSMContext):
    ...
    await administration_page(callback)
...
async def start_administration_work(callback: CallbackQuery, state: FSMContext):
    ...
    if current_state is None:
        await state.update_data(prev=administration_page)
